...
mod tests {
    ...
    /// 3112-byte XMSS signature produced by ream (slot 5, message all-zeros).
    ...
    fn test_cross_client_verify_ream_signature() {
        ...
        assert!(
            sig.is_valid(&pk, REAM_SLOT, &H256::ZERO),
            "ream signature should verify with correct slot and message"
        );
    }
    ...
    fn test_cross_client_ream_signature_rejects_wrong_message() {
        ...
        assert!(
            !sig.is_valid(&pk, REAM_SLOT, &wrong_message),
    ...
    fn test_cross_client_ream_signature_rejects_wrong_slot() {
        let pk = ream_pubkey();
        let sig = ream_signature();
        let wrong_slot = REAM_SLOT + 1;
        assert!(
            !sig.is_valid(&pk, wrong_slot, &H256::ZERO),
            "ream signature should reject wrong slot"
        );
    }
}
